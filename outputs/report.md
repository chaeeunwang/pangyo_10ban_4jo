...
- 핵심 품질 요약 일치: `True`
- 로딩 시간: Pandas `1.575초` / Polars `0.181초`
- 로딩 메모리 사용량: Pandas `450.5MB` / Polars `145.1MB`

> **해석:** 정합성 지표(shape·결측·중복·유효행)는 두 엔진이 동일한 파일을
> 올바르게 읽었는지 확인하는 것일 뿐, 두 라이브러리의 실질적 차이는
> 아니다. 실제 차이는 성능에서 드러나는데, 이번 실행에서는
> `Polars`가 로딩 속도가 약 8.70배 더 빨랐고,
> `Polars`가 메모리 사용량이 약 3.10배 더 적었다.
...
|:-------------------|:--------|-------------------:|------------------:|-----------------:|------------------:|
| Ridge              | False   |             1.0111 |            0.0439 |           0.4514 |           24614.4 |
